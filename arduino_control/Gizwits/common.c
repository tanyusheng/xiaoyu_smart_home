...
#if 0
/**
* @brief String print function
* @param[in]   data  : String data
* @param[in]   dataLen  : String dataLen
* @return     : void
*/
#include <SoftwareSerial.h>
SoftwareSerial mySerial(A2, A3);
extern SoftwareSerial mySerial;
void printf_bufs(uint8_t *data, uint32_t dataLen)
{
    uint32_t i  = 0;

    if(NULL == data)
    {
        mySerial.println(F("!!! printf_bufs Error"));
        return;
    }

    mySerial.print(F("[data:"));mySerial.print(dataLen, DEC);mySerial.print(F("] => "));
    
    for(i=0; i<dataLen; i++)
    {
        mySerial.print(data[i], HEX);mySerial.print(F(" "));
    }
    mySerial.println(F(""));
#endif
