...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagreshiuqi)
    {
        info->event[info->num] = EVENT_reshiuqi;
        info->num++;
        dataPoints->valuereshiuqi = gizStandardDecompressionValue(reshiuqi_BYTEOFFSET,reshiuqi_BITOFFSET,reshiuqi_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagjinhuaqi)
    {
        info->event[info->num] = EVENT_jinhuaqi;
        info->num++;
        dataPoints->valuejinhuaqi = gizStandardDecompressionValue(jinhuaqi_BYTEOFFSET,jinhuaqi_BITOFFSET,jinhuaqi_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagsong)
    {
        info->event[info->num] = EVENT_song;
        info->num++;
        dataPoints->valuesong = gizStandardDecompressionValue(song_BYTEOFFSET,song_BITOFFSET,song_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuereshiuqi != cur->valuereshiuqi)
    {
        GIZWITS_LOG("valuereshiuqi Changed\n");
        ret = 1;
    }
    if(last->valuejinhuaqi != cur->valuejinhuaqi)
    {
        GIZWITS_LOG("valuejinhuaqi Changed\n");
        ret = 1;
    }
    if(last->valuesong != cur->valuesong)
    {
        GIZWITS_LOG("valuesong Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(reshiuqi_BYTEOFFSET,reshiuqi_BITOFFSET,reshiuqi_LEN,(uint8_t *)devStatusPtr,dataPoints->valuereshiuqi);
    gizStandardCompressValue(jinhuaqi_BYTEOFFSET,jinhuaqi_BITOFFSET,jinhuaqi_LEN,(uint8_t *)devStatusPtr,dataPoints->valuejinhuaqi);
    gizStandardCompressValue(song_BYTEOFFSET,song_BITOFFSET,song_LEN,(uint8_t *)devStatusPtr,dataPoints->valuesong);
